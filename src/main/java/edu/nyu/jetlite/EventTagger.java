...
//                d.addF(entitySubTypes.get(i - 1) + "_es_" + entitySubTypes.get(i) + "_es_" + entitySubTypes.get(i + 1));
    ...
    static void findEntityMentions (AceDocument aceDoc, Document doc) {
        ...
        for (Integer tokenStart: entityMentionMap.keySet()) {
            AceEntityMention entityMention = entityMentionMap.get(tokenStart);
            try {
                System.out.println(doc.normalizedText(doc.tokenAt(tokenStart)) + "\t\t" +
                        ((entityMention == null) ? "other" : entityMention.entity.subtype));
